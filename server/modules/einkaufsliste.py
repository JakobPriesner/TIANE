...
def get_aussage_gemeinsam(text, tiane):
    ...
    if 'einkaufsliste' in tiane.local_storage.keys():
        ...
        aussage = tiane.enumerate(einkaufsliste)
    return aussage
    ...
    if 'setz' in text or 'schreib' in text:
        ...
        if 'eigene' in text or 'meine' in text:
            if text != '_UNDO_':
                ...
                if 'einkaufsliste' in nd.keys():
                    ...
                    if double_items:
                        ...
                        tiane.say('Folgende Items befinden sich bereits in deiner Einkaufsliste: {}. Soll ich sie dennoch auf die Einkaufsliste setzen?'.format(tiane.enumerate(double_items)))
                        ...
                        if 'ja' in response or 'gerne' in response:
                            ...
                            if not item:
                                ...
                                tiane.say('Alles klar, nur {} wird auf die Einkaufsliste gesetzt.'.format(tiane.enumerate(item)))
                                for i in item:
                                    nd['einkaufsliste'].append(i)
                    else:
                        for i in item:
                ...
                ausgabe = random.choice(['In Ordnung, ich habe ' + tiane.enumerate(item) + ' zu deiner Einkaufsliste hinzugefügt.',
                                         'Alles klar, ich habe ' + tiane.enumerate(item) + ' auf deine Einkaufsliste gesetzt.',
                                         tiane.enumerate(item) + ' zu deiner Einkaufsliste hinzugefügt.',
                                         'In Ordnung, {}, ich habe '.format(tiane.user) + tiane.enumerate(item) + ' auf deine Einkaufsliste gesetzt.'])
                if not item:
            ...
            if text != '_UNDO_':
                ...
                einkaufsliste = {}
                if 'einkaufsliste' in tiane.local_storage.keys():
                    double_items = get_double_items(item, tiane.local_storage['einkaufsliste'])
                    print(double_items)
                    if double_items:
                        print('Dopplungen gefunden')
                        print(tiane.enumerate(double_items))
                        tiane.say('Folgende Items befinden sich bereits in der Einkaufsliste: {}. Soll ich sie dennoch manche auf die Einkaufsliste setzen?'.format(tiane.enumerate(double_items)))
                        response = tiane.listen()
                        if 'ja' in response or 'gerne' in response:
                            for i in item:
                                tiane.local_storage['einkaufsliste'].append(i)
                        else:
                            for i in double_items:
                                item.remove(i)
                            for i in item:
                ...
                ausgabe = random.choice(['In Ordnung, ich habe ' + tiane.enumerate(item) + ' zur gemeinsamen Einkaufsliste hinzugefügt.',
                                         'Alles klar, ich habe ' + tiane.enumerate(item) + ' auf die gemeinsame Einkaufsliste gesetzt.',
                                         'Alles klar, {}, ich habe '.format(tiane.user) + tiane.enumerate(item) + ' zur gemeinsamen Einkaufsliste hinzugefügt.',
                                         'In Ordnung, {}, ich habe '.format(tiane.user) + tiane.enumerate(item) + ' auf die gemeinsame Einkaufsliste gesetzt.'])
                if not item:
        ...
        if 'meiner' in text or 'eigenen' in text:
            ...
            if aussage != '':
                ausgabe = 'Auf der Liste steht für dich {}, {}.'.format(aussage, tiane.user)
            else:
                ausgabe = random.choice(['Für dich steht aktuell nichts auf der Einkaufsliste.',
                                         'Für dich steht aktuell nichts auf der Einkaufsliste, {}.'.format(tiane.user),
                                         'Für dich steht gerade nichts auf der Einkaufsliste.',
                                         'Für dich steht gerade nichts auf der Einkaufsliste, {}.'.format(tiane.user)])
            tiane.say(ausgabe)
        else:
            ...
            if 'einkaufsliste' in nd.keys():
                ...
                for item in items:
                    try:
                        ...
                        traceback.print_exc()
                        tiane.say('Scheinbar ist {} nicht in der Einkaufsliste vorhanden und konnte daher nicht gelöscht werden.'.format(item))
                    if len(deleted) is not -1:
                        tiane.say(tiane.enumerate(deleted) + ' wurde von deiner Einkaufsliste gelöscht.')
                    else:
                        tiane.say('Da ist wohl was schief gelaufe. Ich konnte leider nichts aus der Einkaufsliste löschen.')
            else:
            ...
            if 'einkaufsliste' in tiane.local_storage.keys():
                ...
                for item in items:
                    try:
                        ...
                        tiane.say('Scheinbar ist {} nicht in der Einkaufsliste vorhanden und konnte daher nicht gelöscht werden.'.format(item))
                    if len(deleted) is not -1:
                        tiane.say(tiane.enumerate(deleted) + ' wurde von der gemeinsamen Einkaufsliste gelöscht.')
                    else:
                        tiane.say('Da ist wohl was schief gelaufe. Ich konnte leider nichts aus der Einkaufsliste löschen.')
            else:
                tiane.say('Ich kann das leider nicht aus deiner Einkaufsliste löschen, da sie leer ist.')
